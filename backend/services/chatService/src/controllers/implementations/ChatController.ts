// import { ChatService } from '../../services/interface/chatServiceInterface';
import { ChatService } from '../../services/interface/IChatService';
import { Message, Participant } from '../../models/chatModel';
import { ChatController } from '../interface/IChatController';
...
export class ChatControllerImpl implements ChatController {
  ...
  async sendMessage(chatId: string, message: Message): Promise<void> {
    await this.chatService.sendMessage(chatId, message);
  }

  async initiateChat(participants: Participant[]): Promise<void> {
    await this.chatService.initiateChat(participants);
  }

  async fetchChat(chatId: string): Promise<void> {
    await this.chatService.fetchChat(chatId);
  }

  async endChat(chatId: string): Promise<void> {
    await this.chatService.endChat(chatId);
  }
}
