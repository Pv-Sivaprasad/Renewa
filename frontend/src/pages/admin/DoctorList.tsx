...
const DoctorList = () => {
  ...
  const toggleBlockStatus = async (userId) => {
    try {
      ...
      if (response) {
        ...
        setUsers(updatedUsers);
        setFilteredDoctors(
          updatedUsers.filter((doctor) =>
            doctor.docname?.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const checkAppointments = (doctorId) => {
          ...
          <div className="flex items-center mb-4">
            <div className="relative w-full">
              <input
                ...
                className="w-1/3 rounded-2xl border border-gray-300 px-3 py-2 text-sm focus:border-blue-500 focus:outline-none"
              />
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredDoctors.map((user, index) => (
                    <tr key={user._id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-100'}>
                      <td className="px-6 py-4 text-sm text-gray-900">{user.docname}</td>
