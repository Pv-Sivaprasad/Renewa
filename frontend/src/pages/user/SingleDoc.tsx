...
import { singleDocData, fetchDoctorReviews } from '../../services/user/userApi';

const DoctorProfile = () => {
  const location = useLocation();
  const docId = location.state.docId;
  const docName=location.state.docName
  console.log('docId in the single doc is', docId);
  console.log('docName in the single doc is', docName);
  ...
  const handleShowReviews = async () => {
    ...
    if (!showReviews) {
      const response = await fetchDoctorReviews(docName);
        ...
        <div className="p-6">
          <div className="flex items-start space-x-6">
            <div className="w-32 h-32 rounded-full overflow-hidden bg-gray-200 flex-shrink-0">
              <img
                src={doctor?.image || '/api/placeholder/128/128'}
                alt={doctor?.docName}
                className="w-full h-full object-cover"
              />
            </div>
            <div className="flex-1">
              <h2 className="text-2xl font-bold mb-2">{doctor?.docName}</h2>
...
//     const fetchDocData=async()=>{
//       let response= await singleDocData(docId)
//       console.log('resposne in frotend is ',response);
//       // setDoctor(response.data)
